 ...
 */
int (stack_empty)(const stack2_t *stk)
{
    assert(stk);
    return (!stk->head);
}
 ...
 */
void (stack_push)(stack2_t *stk, void *data)
{
    struct node *t;
 ...
 */
void *(stack_peek)(const stack2_t *stk)
{
    assert(stk);
    assert(stk->head);
